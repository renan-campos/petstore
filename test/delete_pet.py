#
# Tests the Delete behavior of the petstore web server.
#
...
def test_delete():
    ...
    if (response.status_code != 404):
        return f"Expected pet to have been deleted and therefore not found on GET. Got: {response.status_code}"
    return ""
...
if __name__ == '__main__':
    tests = [
            test("Deleting a pet that doesn't exist", test_nodelete),
            test("Creating a pet", test_create),
            test("Deleting a pet that does exist", test_delete),
            ]
    tests_passed = True
